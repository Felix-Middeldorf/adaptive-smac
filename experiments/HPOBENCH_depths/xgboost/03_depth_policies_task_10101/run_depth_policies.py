...
def depth_for_trial(policy: str, completed_trials: int) -> int:
    ...
    if policy == "staged_80_50_rest":
        return 4 if completed_trials < 80 else 8 if completed_trials < 130 else 20
...
def run_smac(smac_seed: int, policy: str) -> dict[str, Any]:
    benchmark = XGBoostBenchmarkBB(task_id=TASK_ID, rng=BENCHMARK_SEED, bind_str=CONTAINER_PATCH)
...
def submit_jobs() -> None:
    ...
    executor.update_parameters(
        ...
        cpus_per_task=1,
        mem_gb=4,
        slurm_job_name="HPOBench_XGB_DepthPolicies",
