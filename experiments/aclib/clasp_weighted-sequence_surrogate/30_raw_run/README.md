# Clasp weighted-sequence raw SMAC timing run
...
(the median prediction). The reproducible sampled initial configuration from
`01_initial_cw` is used instead of the default.
...
default account, with 16 hours and 4 GB per job. This timing control intentionally preserves the installed
benchmark behavior, including its currently identified cutoff-boundary issue.
