# Lingeling CircuitFuzz: initial fixed-depth experiment
...
- all 299 training instances and no test instances
- Slurm account `lect0190` and worker `PYTHONHASHSEED=0`

The ACLib default ranked first in a 129-configuration screen, so the run starts
from the reproducible first random draw recorded in `initial_config.json`.
This avoids giving SMAC an already exceptional incumbent without cherry-picking
a deliberately bad replacement.
