...
def clone_runhistory(runhistory: RunHistory, configspace: ConfigurationSpace) -> RunHistory:
    ...
    tmp_file = tempfile.NamedTemporaryFile(
        prefix="hartmann_branch_select_leaf_rh_",
        suffix=".json",
        delete=False,
    )
...
def run_branch(
    ...
    branch_name = f"leaf_{min_samples_leaf}"
    ...
    model = HPOFacade.get_model(scenario=scenario, min_samples_leaf=min_samples_leaf)
    smac = HPOFacade(
        scenario=scenario,
...
def run_seed(seed: int) -> dict[str, Any]:
    ...
    Actual function evaluations: 5 leaf_values * 100 trials * 5 blocks = 2500.
    """
    ...
    for block_idx in range(N_BLOCKS):
        ...
        selected_leaf_values.append(int(best_branch["min_samples_leaf"]))
        ...
        print(
            f"[seed={seed}] selected min_samples_leaf={best_branch['min_samples_leaf']} "
    ...
    summary = {
        "seed": seed,
        "leaf_values": MIN_SAMPLES_LEAF_VALUES,
        "trials_per_block": TRIALS_PER_BLOCK,
        "n_blocks": N_BLOCKS,
        "selected_leaf_values": selected_leaf_values,
        "selected_trials": N_SELECTED_TRIALS,
        "actual_evaluations": len(MIN_SAMPLES_LEAF_VALUES) * TRIALS_PER_BLOCK * N_BLOCKS,
    ...
    print(f"[seed={seed}] selected_leaf_values={selected_leaf_values}")
...
def submit_jobs() -> None:
    ...
    executor.update_parameters(
        ...
        cpus_per_task=1,
        mem_gb=2.4,
        slurm_job_name="HartmannLeafBranchSelect",
