...
TIMEOUT_MIN = 4 * 60
MEM_GB = 4
LOG_DIRECTORY = HERE / "submitit_logs_extended_fixed"
...
def experiment_runs() -> tuple[RunSpec, ...]:
    return tuple(
        ...
        for policy in EXTENDED_FIXED_POLICIES
...
def experiment_packs() -> tuple[RunPack, ...]:
    ...
    if flattened != list(runs) or len(set(flattened)) != len(runs):
        raise RuntimeError("Packed extension runs are missing, reordered, or duplicated.")
    if len(packs) > MAX_SUBMITTED_JOBS:
        raise RuntimeError(
            f"Extension has {len(packs)} jobs, above limit {MAX_SUBMITTED_JOBS}."
        )
    return packs
...
def print_experiment_summary(*, list_packs: bool = False) -> None:
    ...
    runs = experiment_runs()
    print(
        f"Extended fixed policies ({len(EXTENDED_FIXED_POLICIES)}): "
        f"{[policy.name for policy in EXTENDED_FIXED_POLICIES]}"
    )
    ...
    print(f"New SMAC runs: {len(runs)}")
...
def submit_jobs() -> None:
    ...
    executor.update_parameters(
        ...
        cpus_per_task=1,
        mem_gb=MEM_GB,
        slurm_job_name="SynthACtic_O1_D15_I20_Fixed32to50",
        slurm_setup=[
...
def main() -> None:
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Validate and summarize the extension without submitting.",
    )
    parser.add_argument(
        "--list-packs",
        action="store_true",
