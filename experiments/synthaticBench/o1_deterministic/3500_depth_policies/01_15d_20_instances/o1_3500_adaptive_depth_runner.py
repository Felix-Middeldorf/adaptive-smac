...
def all_policy_specs() -> tuple[PolicySpec, ...]:
    ...
    staged = (
        PolicySpec("a1", "adaptive_a", "staged", has_refinement=False),
        PolicySpec("a2", "adaptive_a", "staged", has_refinement=True),
        PolicySpec(
            "b1", "adaptive_b", "staged", add_middle=True,
            has_refinement=False,
        ),
        PolicySpec(
            "b2", "adaptive_b", "staged", add_middle=True,
            has_refinement=True,
        ),
    )
    expanding = (
        PolicySpec("c1", "adaptive_c", "expanding", None, (4, 6, 8, 10), 2),
        PolicySpec("c2", "adaptive_c", "expanding", None, (5, 7, 9), 2),
        PolicySpec("c3", "adaptive_c", "expanding", None, (4, 7, 10), 3),
        PolicySpec(
            "e1", "adaptive_e", "expanding", None, (4, 6, 8, 10), 2,
            strict=True,
        ),
        PolicySpec(
            "e2", "adaptive_e", "expanding", None, (5, 7, 9), 2,
            strict=True,
        ),
        PolicySpec(
            "e3", "adaptive_e", "expanding", None, (4, 7, 10), 3,
            strict=True,
        ),
    )
    other = (PolicySpec("g", "adaptive_g", "g"),)
    policies = fixed + staged + expanding + other
    if len(policies) != 15 or len({policy.name for policy in policies}) != 15:
        raise RuntimeError("The explicit specification contains 15 unique policies.")
    return policies
...
class DepthPolicyController:
    ...
    def _handle_g_boundary(self, boundary: int, completed_trials: int) -> None:
        ...
        if boundary == 2100:
            current_depth = self._last_depth or self.depths[0]
            neighborhood = (
                max(1, current_depth - 4), current_depth, current_depth + 4
            )
            self.selections.append(
                {
                    "scheduled_boundary": boundary,
