...
def dimension_name(dimension: int) -> str:
...
def trajectory_is_complete(
    ...
    path = trajectory_path(dimension, benchmark_seed, depth, smac_seed)
    if not path.exists():
        return False
    try:
...
def run_fixed_depth(
    ...
    if dimension not in DIMENSIONS:
        raise ValueError(f"Expected dimension from {DIMENSIONS}, got {dimension}.")
    ...
    if trajectory_is_complete(dimension, benchmark_seed, depth, smac_seed):
        print(
            f"Skipping complete dimension={dimension}, "
            f"benchmark_seed={benchmark_seed}, depth={depth}, smac_seed={smac_seed}."
        )
        return json.loads(
            trajectory_path(dimension, benchmark_seed, depth, smac_seed).read_text()
        )
...
def run_depth_batch(
    ...
    return [
        run_fixed_depth(dimension, benchmark_seed, smac_seed, depth)
        for depth in depths
    ]
