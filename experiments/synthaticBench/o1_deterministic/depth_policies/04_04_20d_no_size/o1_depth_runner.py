...
class ThreeStageDepthCallback(Callback):
    ...
    def on_next_configurations_start(self, config_selector) -> None:
        ...
        rf_options = config_selector._model._rf_opts
        rf_options["max_depth"] = depth
        if (
            rf_options["min_samples_leaf"] != MIN_SAMPLES_LEAF
            or rf_options["min_samples_split"] != MIN_SAMPLES_SPLIT
        ):
            raise RuntimeError("The RF size settings changed during the run.")
...
def _run(
    ...
    name = f"fixed_depth_{schedule[0]}" if is_fixed else policy_name(schedule)
    ...
    model = ACFacade.get_model(
        scenario=scenario,
        max_depth=schedule[0],
        min_samples_leaf=MIN_SAMPLES_LEAF,
        min_samples_split=MIN_SAMPLES_SPLIT,
    )
    if (
        model._rf_opts["min_samples_leaf"] != MIN_SAMPLES_LEAF
        or model._rf_opts["min_samples_split"] != MIN_SAMPLES_SPLIT
    ):
        raise RuntimeError("SMAC did not apply the requested RF size settings.")
