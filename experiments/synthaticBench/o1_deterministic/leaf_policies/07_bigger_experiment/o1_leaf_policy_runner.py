...
class LeafPolicy:
    ...
    stage_leaves: tuple[int, ...] = ()

    def leaf(self, completed_trials: int) -> int:
        if completed_trials < 0:
            raise ValueError("completed_trials must be non-negative.")
        if self.schedule_type == "fixed":
            if self.fixed_leaf is None:
                raise ValueError("A fixed policy requires fixed_leaf.")
            value = self.fixed_leaf
        elif self.schedule_type == "three_stage":
            if len(self.stage_leaves) != 3:
                raise ValueError("A three-stage policy requires three leaf values.")
            if completed_trials < STAGE_SWITCHES[0]:
                value = self.stage_leaves[0]
            elif completed_trials < STAGE_SWITCHES[1]:
                value = self.stage_leaves[1]
            else:
                value = self.stage_leaves[2]
        else:
            raise ValueError(f"Unknown schedule type {self.schedule_type!r}.")
        if value not in LEAF_VALUES:
            raise ValueError(f"Leaf value {value} is outside {LEAF_VALUES}.")
        return value
    ...
    def to_spec(self) -> dict[str, Any]:
        return {
            ...
            "fixed_leaf": self.fixed_leaf,
            "stage_leaves": list(self.stage_leaves),
    ...
    def planned_transitions(self) -> list[list[int]]:
        ...
        for completed_trials in range(N_TRIALS):
            value = self.leaf(completed_trials)
            if value != previous:
...
def all_policies() -> tuple[LeafPolicy, ...]:
    fixed = tuple(
        LeafPolicy(
            name=f"fixed_leaf_{value}",
            family="fixed",
            schedule_type="fixed",
            fixed_leaf=value,
        )
    ...
    staged = tuple(
        LeafPolicy(
            name="leaf_stages_" + "_".join(map(str, values)),
            family="three_stage",
            schedule_type="three_stage",
            stage_leaves=values,
        )
        for values in product(LEAF_VALUES, repeat=3)
        if len(set(values)) > 1
    )
    policies = fixed + staged
    if len(policies) != 27 or len({policy.name for policy in policies}) != 27:
        raise RuntimeError("Expected 27 unique leaf policies.")
    if Counter(policy.family for policy in policies) != Counter(
        {"fixed": 3, "three_stage": 24}
    ):
        raise RuntimeError("Unexpected leaf-policy family counts.")
    return policies


class LeafPolicyCallback(Callback):
    def __init__(self, policy: LeafPolicy) -> None:
        super().__init__()
        self.policy = policy
        self._last_leaf = policy.leaf(0)
        self.observed_transitions: list[list[int]] = [[0, self._last_leaf]]

    def on_next_configurations_start(self, config_selector: Any) -> None:
        completed_trials = len(config_selector._runhistory)
        value = self.policy.leaf(completed_trials)
        config_selector._model._rf_opts["min_samples_leaf"] = value
        if value != self._last_leaf:
            self.observed_transitions.append([completed_trials, value])
            self._last_leaf = value
            print(
                f"[LeafPolicy] completed_trials={completed_trials}, "
                f"min_samples_leaf={value}"
            )
...
def run_policy(
    ...
    model = ACFacade.get_model(
        scenario=scenario,
        max_depth=MAX_DEPTH,
        min_samples_leaf=policy.leaf(0),
    ...
    result = {
        ...
        "initial_min_samples_leaf": policy.leaf(0),
        "final_min_samples_leaf": policy.leaf(N_TRIALS - 1),
