...
PREVIOUS_SPLIT_VALUES = (2, 3)
N_DIVERSE_STAGED_POLICIES = 25
...
def diverse_staged_values() -> tuple[tuple[int, int, int], ...]:
    candidates = tuple(
        values
        for values in product(SPLIT_VALUES, repeat=3)
        if len(set(values)) > 1
        and any(value not in PREVIOUS_SPLIT_VALUES for value in values)
    )
    selected: list[tuple[int, int, int]] = []
    covered_pairs: set[tuple[int, int, int, int]] = set()
    marginal_counts: Counter[tuple[int, int]] = Counter()
    stage_pairs = ((0, 1), (0, 2), (1, 2))

    # Greedily fill uncovered stage-pair cells, then maximize distance and balance.
    while len(selected) < N_DIVERSE_STAGED_POLICIES:
        def score(values: tuple[int, int, int]) -> tuple[Any, ...]:
            pairs = {
            ...
            min_distance = (
                ...
                if not selected
                    ...
                    sum(left != right for left, right in zip(values, chosen))
                    for chosen in selected
                )
            )
            updated_counts = marginal_counts.copy()
            updated_counts.update(enumerate(values))
            imbalance = sum(
                updated_counts[(stage, value)] ** 2
            ...
            return (
                ...
                min_distance,
                sum(
                    (stage, value) not in marginal_counts
        ...
        chosen = max(
            (values for values in candidates if values not in selected),
            key=score,
        )
        ...
        marginal_counts.update(enumerate(chosen))

    expected_pairs = {
        (left, left_value, right, right_value)
        for left, right in stage_pairs
        for left_value in SPLIT_VALUES
        for right_value in SPLIT_VALUES
    }
    ...
    if covered_pairs != expected_pairs:
        raise RuntimeError("Diverse policies do not cover every stage-pair cell.")
    if adjacent_transitions != set(product(SPLIT_VALUES, repeat=2)):
        raise RuntimeError("Diverse policies do not cover every adjacent transition.")
    if set(marginal_counts.values()) != {6, 7}:
        raise RuntimeError("Diverse policies do not have balanced stage marginals.")
...
def all_policies() -> tuple[SplitPolicy, ...]:
    ...
    staged = tuple(
        ...
        for values in diverse_staged_values()
    )
    policies = fixed + staged
    if len(policies) != 27 or len({policy.name for policy in policies}) != 27:
        raise RuntimeError("Expected 27 unique additional split policies.")
    if Counter(policy.family for policy in policies) != Counter(
        {"fixed": 2, "three_stage": N_DIVERSE_STAGED_POLICIES}
    ):
