...
def run_policy(
    ...
    if policy.startswith("fixed_depth_"):
        ...
        if initial_depth not in FIXED_DEPTHS:
            raise ValueError(f"Unsupported fixed depth: {initial_depth}")
        callback = None
...
def submit_jobs() -> None:
    ...
    executor.update_parameters(
        ...
        cpus_per_task=1,
        mem_gb=4,
        slurm_job_name="SynthACtic_O6_DepthCounterCheck",
    ...
    print(f"Submitted {len(jobs)} counter-check jobs:")
