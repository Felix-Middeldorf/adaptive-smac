...
def submit_jobs() -> None:
    ...
    executor.update_parameters(timeout_min=50, slurm_partition="c23ms", slurm_array_parallelism=25, cpus_per_task=1, mem_gb=4, slurm_job_name="SynthACtic_O6_D50_FixedDepth", slurm_setup=[f"export PYTHONHASHSEED={PYTHONHASHSEED}"], slurm_additional_parameters={"requeue": True})
    jobs = []
    with executor.batch():
        for depth in DEPTHS:
